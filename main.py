...
osc100 = Oscilloscope(gui=gui, input_ops=[(midi, 0)], name='MIDI Freq')
osc101 = Oscilloscope(gui=gui, input_ops=[(midi, 1)], name='MIDI Amp')
osc1 = Oscillator(input_ops=[(midi, 0), (midi, 1), (c3_phi, 0)],
                  osc_type='square',
                  name='Osc1')

raw_osc = Oscilloscope(input_ops=[(osc1, 0)], gui=gui, name='Before-filter Oscilloscope')
filtered = FIRFilter(input_ops=[(osc1, 0)],
                     ...
                     gui=gui,
                     name='Filter1')
osc = Oscilloscope(input_ops=[(filtered, 0)], gui=gui, name='After-filter Oscilloscope')
